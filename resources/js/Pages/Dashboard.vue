    ...
    <AdminLayout>
        ...
        <div class="flex flex-wrap items-end justify-between gap-4">
            <div class="min-w-0">
                <div class="flex flex-wrap items-center gap-2 text-sm text-slate-500 dark:text-slate-400">
                    ...
                    <span class="inline-flex items-center rounded-full bg-teal-50 px-2.5 py-0.5 text-xs font-semibold text-teal-800 dark:bg-teal-500/15 dark:text-teal-200">
                        {{ branchName }}
                    </span>
                </div>
                <h1 class="mt-2 text-3xl font-semibold tracking-tight">{{ greeting }}, {{ userName }}</h1>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
                    Ringkasan cabang ini — omzet, stok, dan transaksi hari ini.
                </p>
            </div>
            <div class="flex flex-wrap gap-2">
                <Link href="/reports" class="btn-secondary">Laporan</Link>
                <Link href="/pos" target="_blank" rel="noopener" class="btn-primary">Buka Kasir</Link>
            </div>
        </div>

        <div class="mt-8 grid gap-4 xl:grid-cols-12">
            <section class="relative overflow-hidden rounded-[28px] bg-slate-900 p-7 text-white shadow-[0_20px_50px_rgb(15,23,42,0.18)] xl:col-span-7 dark:bg-[#0f1729]">
                ...
                <div class="relative flex h-full flex-col justify-between gap-8">
                    <div class="flex items-start justify-between gap-4">
                        <div>
                            ...
                            <p class="mt-3 text-4xl font-bold tracking-tight sm:text-5xl">{{ formatRupiah(stats.revenue_today) }}</p>
                        </div>
                        <span class="rounded-full bg-white/10 px-3 py-1 text-[11px] font-semibold uppercase tracking-wide text-teal-100">
                            Live
                        </span>
                    </div>
                    <div class="grid gap-4 sm:grid-cols-3">
                        <div class="rounded-2xl bg-white/8 px-4 py-3 ring-1 ring-white/10">
                            <p class="text-[11px] uppercase tracking-wide text-slate-400">Rata-rata nota</p>
                            <p class="mt-1 text-lg font-semibold">{{ formatRupiah(averageTicket) }}</p>
                        </div>
                        <div class="rounded-2xl bg-white/8 px-4 py-3 ring-1 ring-white/10">
                            <p class="text-[11px] uppercase tracking-wide text-slate-400">Transaksi</p>
                            <p class="mt-1 text-lg font-semibold">{{ formatNumber(stats.transactions_today) }}</p>
                        </div>
                        <div class="rounded-2xl bg-white/8 px-4 py-3 ring-1 ring-white/10">
                            <p class="text-[11px] uppercase tracking-wide text-slate-400">7 hari</p>
                            <p class="mt-1 text-lg font-semibold">{{ formatRupiah(weekRevenue) }}</p>
                        </div>
                    </div>
                </div>
            </section>

            <div class="grid gap-4 sm:grid-cols-2 xl:col-span-5 xl:grid-cols-1">
                <section class="admin-card flex items-center justify-between gap-4 p-6">
                    <div class="min-w-0">
                        <p class="text-sm text-slate-500 dark:text-slate-400">Laba kotor</p>
                        <p class="mt-2 text-2xl font-bold tracking-tight">{{ formatRupiah(stats.profit_today) }}</p>
                        <p class="mt-2 text-xs text-slate-400">Margin {{ profitMargin }}% dari omzet.</p>
                    </div>
                    <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-emerald-50 text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-200">
                ...
                <section class="admin-card flex items-center justify-between gap-4 p-6">
                    <div class="min-w-0">
                        ...
                        <p class="mt-2 text-2xl font-bold tracking-tight" :class="stats.low_stock ? 'text-amber-600 dark:text-amber-400' : ''">
                    ...
                    <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl" :class="stats.low_stock ? 'bg-amber-50 text-amber-700 dark:bg-amber-500/15 dark:text-amber-200' : 'bg-teal-50 text-teal-700 dark:bg-teal-500/15 dark:text-teal-200'">
        ...
        <div class="mt-4 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            <Link
                ...
                :key="tile.label"
                :href="tile.href"
                class="admin-card group flex items-center gap-4 p-5 transition hover:-translate-y-0.5"
            >
                <div class="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl" :class="iconTones[tile.tone]">
                    <SidebarIcon :name="tile.icon" />
                </div>
                <div class="min-w-0">
                    <p class="text-xs font-medium text-slate-500 dark:text-slate-400">{{ tile.label }}</p>
                    <p class="mt-0.5 truncate text-lg font-bold tracking-tight">{{ tile.value }}</p>
        ...
        <div class="mt-6 grid gap-4 xl:grid-cols-12">
            <section class="admin-card p-6 xl:col-span-8">
                ...
                <div class="mt-6 flex h-48 items-end gap-2 sm:gap-3">
                    ...
                    >
                        <p class="text-[10px] font-medium text-slate-400 opacity-0 transition group-hover:opacity-100">
                            {{ formatRupiah(day.revenue) }}
                        </p>
                        <div class="flex h-32 w-full items-end overflow-hidden rounded-2xl bg-slate-100 dark:bg-white/5">
                            <div
                                class="w-full rounded-2xl transition duration-300 group-hover:brightness-110"
                        ...
                        <p class="text-[11px] font-medium" :class="isToday(day) ? 'text-teal-700 dark:text-teal-300' : 'text-slate-500 dark:text-slate-400'">
                            {{ day.label }}
                        </p>
                    </div>
                </div>
            ...
            <section class="admin-card p-6 xl:col-span-4">
                ...
                <div class="mt-5 grid grid-cols-2 gap-2">
                    <Link
                        ...
                        :key="item.href"
                        :href="item.href"
                        class="flex items-center gap-2 rounded-2xl bg-slate-50 px-3 py-3 text-sm font-medium text-slate-700 transition hover:bg-teal-50 hover:text-teal-800 dark:bg-white/5 dark:text-slate-200 dark:hover:bg-teal-500/10 dark:hover:text-teal-200"
        ...
        <div class="mt-6 grid gap-6 xl:grid-cols-5">
            <section class="admin-card p-6 xl:col-span-3">
                <div class="flex items-center justify-between gap-3">
                    <div>
                        <h2 class="font-semibold">Transaksi terbaru</h2>
                        <p class="mt-1 text-xs text-slate-400">Nota selesai di cabang ini.</p>
                    </div>
                    <Link href="/sales" class="text-xs font-semibold text-teal-700 dark:text-teal-300">Lihat semua</Link>
                </div>
                <div v-if="recentSales.length" class="mt-4 divide-y divide-slate-100 dark:divide-white/5">
                    <Link
                        ...
                        :key="sale.id"
                        :href="`/sales/${sale.id}`"
                        class="-mx-2 flex items-center gap-4 rounded-2xl px-2 py-3 transition first:pt-3 last:pb-3 hover:bg-slate-50 dark:hover:bg-white/5"
                    >
                        <div class="flex h-11 w-11 items-center justify-center rounded-2xl bg-slate-100 text-xs font-bold text-slate-600 dark:bg-white/10 dark:text-slate-200">
                        ...
                        <StatusBadge class="max-sm:hidden">{{ paymentMethodsLabel(sale) }}</StatusBadge>
                        <p class="w-28 shrink-0 text-right text-sm font-bold">{{ formatRupiah(sale.grand_total) }}</p>
                    </Link>
                </div>
                <div v-else class="mt-8 rounded-2xl border border-dashed border-slate-200 px-4 py-12 text-center dark:border-white/10">
            ...
            <section class="admin-card p-6 xl:col-span-2">
                    ...
                    <div v-for="(item, index) in topProducts" :key="item.name">
                        <div class="flex items-center justify-between gap-3">
                            <div class="flex min-w-0 items-center gap-3">
                                <div class="flex h-9 w-9 items-center justify-center rounded-2xl text-xs font-bold" :class="productTone(item.name)">
                                    {{ index + 1 }}
                                </div>
                                <div class="min-w-0">
                            ...
                            <p class="text-sm font-semibold">{{ formatRupiah(item.revenue) }}</p>
                        </div>
                ...
                <div v-else class="mt-8 rounded-2xl border border-dashed border-slate-200 px-4 py-12 text-center text-sm text-slate-500 dark:border-white/10">
        ...
        <section class="admin-card mt-6 p-6">
            <div class="flex items-center justify-between gap-3">
                ...
                <Link href="/stock" class="text-xs font-semibold text-teal-700 dark:text-teal-300">Kelola stok</Link>
            </div>
            <div v-if="lowStock.length" class="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                <div
                ...
                >
                    ...
                    <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">{{ row.product?.sku }}</p>
